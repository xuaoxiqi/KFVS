...
module flux
    ...
    contains
        ...
        subroutine calc_flux_boundary(cell_L,face,cell_R,idx)
            ...
            if(idx.EQ.IDIRC) then
                ...
                ! non-penetration B.C.
                cell_R%prim(2) = -cell_L%prim(2)/cell_L%prim(1)*cell_R%prim(1)
                cell_R%prim(3) = cell_L%prim(3)/cell_L%prim(1)*cell_R%prim(1)
                ! non-penetration B.C.
        ...
        subroutine timestep()
            ...
            do j=iymin,iymax
                do i=ixmin,ixmax
                    ...
                    !maximum velocity
                    prim(2) = Sqrt(prim(2)**2+prim(3)**2)+Sqrt(prim(1))
        ...
        subroutine evolution()
            ...
            do j=iymin,iymax
                do i=ixmin,ixmax+1 !with ghost cell
                    ...
                    if(i.EQ.250) then
                        if( (j.LE.238).OR.(j.GT.425) ) call calc_flux_boundary(ctr(i-1,j),vface(i,j),ctr(i,j),IDIRC)
                    endif
                end do
            enddo
...
module io
    ...
    contains
        ...
        !--------------------------------------------------
        subroutine init()
            real(kind=RKD) :: higher_gas(3), lower_gas(3) !initial condition
            integer :: xnum,ynum

            !control
            cfl = 0.1 !CFL number
            write(*,*) "CFL =",cfl
            max_time = 7.2d0 !output time interval
            ...
            higher_gas = [1.0, 0.0, 0.0]
            lower_gas = [0.5, 0.0, 0.0]

            call init_geometry(xnum,ynum) !initialize the geometry
            call init_flow_field(higher_gas,lower_gas) !set the initial condition
        end subroutine init
        ...
        subroutine init_geometry(xnum,ynum)
            ...
            dx = 20.0d0/(ixmax-ixmin+1)
            dy = 20.0d0/(iymax-iymin+1)
            write(*,*) "dx=",dx,",  dy=",dy
...
!            cell center (with ghost cell)
        ...
        !--------------------------------------------------
        subroutine init_flow_field(higher_gas,lower_gas)
            real(kind=RKD),intent(in) :: higher_gas(3),lower_gas(3)
            real(kind=RKD) :: w_higher(3),w_lower(3)
            integer :: i,j
            ...
            do j=iymin-1,iymax+1
                do i=ixmin-1,ixmax+1
                    if ( i.LE.nxHalf ) then
                        ctr(i,j)%prim = higher_gas
                    else
                        ctr(i,j)%prim = lower_gas
                    endif
                enddo
            enddo


            w_higher = get_conserved(higher_gas)
            w_lower = get_conserved(lower_gas)
            do j=iymin-1,iymax+1
                do i=ixmin-1,ixmax+1
                    if ( i.LE.nxHalf ) then
                        ctr(i,j)%w = w_higher
